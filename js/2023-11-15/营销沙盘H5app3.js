...
function openApp(){
    ...
    selector().text('工作助手').depth(9).waitFor();
...
function gesture_verification(){
    for(var i=0;i<3;i++){
        automator.gesture(6000, [237,929],[237,1253],[237,1577],[561,1100],[885,900]);
...
function exitApp(){
    var xj_str = ''
    try{
        selector().text('工单').findOne(3000).prev().click()
        sleep(2000)
        var el = selector().className('android.widget.Button').findOne(3000)
        var x=el.centerX() - 120
        var y=el.centerY()
        automator.click(x, y)
        console.log('----开始退出账号----')
        sleep(3000)
